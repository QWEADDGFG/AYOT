...
InferenceConfig::InferenceConfig() : modelWidth(800), modelHeight(800),
                                     confidenceThreshold(0.001), nmsThreshold(0.45),
                                     modelOutputBoxNum(13125), classNum(5) {}
...
vector<OBBBoundingBox> OBBPostProcessor::Tensor2Boxes(float* outputData, int srcWidth, int srcHeight, int modelWidth, int modelHeight, float confidenceThreshold)
{
    ...
    for (size_t i = 0; i < modelOutputBoxNum_; ++i)
    {
        ...
        if (maxValue >= confidenceThreshold && score >= confidenceThreshold)
        {
            ...
            angle = outputData[(9 + classNum_) * modelOutputBoxNum_ + i]; // [-pi/2, pi/2] //longside ？？？角度输出归一化到 [0, 1]
...
bool OBBNMSProcessor::IoUbyOBBandHBB(float& cx, float& cy, float& cw, float& ch, float& angle, float x, float y, float w, float h, float thresh)
{
    float cos_a = cos(angle);
    float sin_a = sin(angle);
    float w_half = cw / 2.0f;
    float h_half = ch / 2.0f;

    float dx1 = cx + -w_half * cos_a + h_half * sin_a;
    float dy1 = cy + -w_half * sin_a - h_half * cos_a;

    float dx2 = cx + w_half * cos_a + h_half * sin_a;
    float dy2 = cy + w_half * sin_a - h_half * cos_a;

    float dx3 = cx + w_half * cos_a - h_half * sin_a;
    float dy3 = cy + w_half * sin_a + h_half * cos_a;

    float dx4 = cx + -w_half * cos_a - h_half * sin_a;
    float dy4 = cy + -w_half * sin_a + h_half * cos_a;

    float cx1 = min4(dx1, dx2, dx3, dx4);
    float cx2 = max4(dx1, dx2, dx3, dx4);
    float cy1 = min4(dy1, dy2, dy3, dy4);
    float cy2 = max4(dy1, dy2, dy3, dy4);
    ...
    dy3 = std::max(dy1, cy1);
    dy4 = std::min(dy2, cy2);
    float inner_area = std::max(0.0f, dx4 - dx3+1.0f) * std::max(0.0f, dy4 - dy3+1.0f);
    float area1 = cw * ch;
    float area2 = w * h;
    float iou = inner_area / (area1 + area2 - inner_area + 1e-7f);
