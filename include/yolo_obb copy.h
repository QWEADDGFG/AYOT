...
public:
    ...
    // YOLOV8_OBB 格式的后处理，添加缩放信息参数
    vector<OBBBoundingBox> parseOutput(float *outputData, 
                                       const ScaleInfo& scaleInfo,
                                       float confidenceThreshold);
    
    // YOLO11_OBB 格式的后处理，添加缩放信息参数
    vector<OBBBoundingBox> parseOutput_YOLO11OBB(float* data, 
                                        const ScaleInfo& scaleInfo,
                                        float confidenceThreshold);
                                        
private:
    // 将模型输出坐标转换为原图坐标
    void convertToOriginalCoords(OBBBoundingBox& box, const ScaleInfo& scaleInfo);
};
...
private:
    // 计算缩放信息
    ScaleInfo calculateScaleInfo(int originalWidth, int originalHeight);
    // 等比例缩放并填充
    bool resizeKeepAspect(const ImageData& input, ImageData& output);
    InferenceConfig config_;
    AclLiteResource aclResource_;
    AclLiteImageProc imageProcess_;
    AclLiteModel model_;
